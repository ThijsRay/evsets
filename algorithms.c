...
int naive_eviction_optimistic(EvsetsElement **ptr, EvsetsElement **can, char *victim) {
  EvsetsElement *candidate = NULL, *es = NULL;
...
int gt_eviction(EvsetsElement **ptr, EvsetsElement **can, char *victim) {
  ...
  EvsetsElement **chunks = (EvsetsElement **)calloc(conf.cache_way + 1, sizeof(EvsetsElement *));
  if (!chunks) {
    return 1;
  }
  ...
  // Backtrack record
  EvsetsElement **back = (EvsetsElement **)calloc(h * 2, sizeof(EvsetsElement *)); // TODO: check height bound
  if (!back) {
    free(chunks);
    free(ichunks);
    return 1;
...
int gt_eviction_any(EvsetsElement **ptr, EvsetsElement **can) {
  EvsetsElement **chunks = (EvsetsElement **)calloc(conf.cache_way + 2, sizeof(EvsetsElement *));
  if (!chunks) {
    return 1;
  }
  // Random chunk selection
  ...
  // Backtrack record
  EvsetsElement **back = calloc(h * 2, sizeof(EvsetsElement *)); // TODO: check height bound
  if (!back) {
    free(chunks);
    free(ichunks);
    return 1;
  ...
  do {
    ...
    while (len > conf.cache_way + 1) {
      ...
      do {
        ...
        n = n + 1;
      } while (!(ret = (test_and_time(*ptr, conf.rounds, conf.threshold, conf.cache_way, conf.traverse))) &&
               (n < conf.cache_way + 2));
