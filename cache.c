...
inline void traverse_list_skylake(EvsetsElement *ptr) {
...
inline void traverse_list_asm_skylake(EvsetsElement *ptr) {
...
inline void traverse_list_asm_haswell(EvsetsElement *ptr) {
  __asm__ volatile("test %%rcx, %%rcx;"
                   "jz out2;"
                   "loop2:"
                   "movq (%%rcx), %%rax;"
...
inline void traverse_list_asm_simple(EvsetsElement *ptr) {
...
inline void traverse_list_haswell(EvsetsElement *ptr) {
  while (ptr && ptr->next) {
    maccess(ptr);
    maccess(ptr->next);
    maccess(ptr);
...
inline void traverse_list_simple(EvsetsElement *ptr) {
  while (ptr) {
    maccess(ptr);
    ptr = ptr->next;
  }
...
inline void traverse_list_to_n(EvsetsElement *ptr, int n) {
  while (ptr && n-- > 0) {
    maccess(ptr);
    ptr = ptr->next;
  }
...
inline void traverse_list_time(EvsetsElement *ptr, void (*trav)(EvsetsElement *)) {
  size_t time;
  trav(ptr);
  while (ptr) {
    //		time = rdtsc();
...
int tests_avg(EvsetsElement *ptr, char *victim, int rep, int threshold, void (*trav)(EvsetsElement *)) {
  ...
  EvsetsElement *vic = (EvsetsElement *)victim;
  vic->delta = 0;
  for (i = 0; i < rep; i++) {
...
int calibrate(char *victim, struct evsets_config *conf) {
  ...
  if (conf->flags & FLAG_VERBOSE) {
    printf("\tflushed: min %d, mode %d, avg %f, max %d, std %.02f, q %d (%.02f)\n", hist_min(flushed, hsz),
           hist_mode(flushed, hsz), hist_avg(flushed, hsz), hist_max(flushed, hsz),
           hist_std(flushed, hsz, hist_avg(flushed, hsz)), hist_q(flushed, hsz, ret),
