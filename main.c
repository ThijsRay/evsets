...
struct evsets_config configuration = {
    .rounds = 10,
    .cal_rounds = 1000000,
    .stride = 4096,
    .cache_size = 6 << 20,
...
void usage(char *name) {
  printf("[?] Usage: %s [flags] [params]\n\n"
         ...
         "\t\t-h\t\tshow this help\n\n"
         "\tExample:\n\t\t%s -b 3000 -c 6 -s 8 -a g -n 12 -o 4096 -e 2 -C 0 -t 85 --verbose --retry "
         "--backtracking\n"
         "\n",
         name, name);
}
...
int main(int argc, char **argv) {
  ...
  while ((option = getopt_long(argc, argv, "hb:t:q:c:s:n:o:a:e:r:C:x:y:", long_options, &option_index)) !=
         -1) {
    switch (option) {
    case 0:
      break;
