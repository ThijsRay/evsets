...
void recheck(EvsetsElement *ptr, char *victim, bool err, struct evsets_config *conf) {
  ...
  if (vpaddr == 0xffffffffffffffff) {
    ...
    if (!err) {
      ...
      if (victim) {
        printf(" - victim pfn: 0x%llx, cache set: 0x%llx, slice: ", vpaddr, vcacheset);
  ...
  while (ptr) {
    ...
    if (!err) {
      printf(" - element pfn: 0x%llx, cache set: 0x%llx, slice: ", paddr, cacheset);
      if (!(conf->flags & FLAG_IGNORESLICE)) {
        printf("0x%llx\n", slice);
      } else {
        printf("???\n");
...
int filter(EvsetsElement **ptr, char *victim, int n, int m, struct evsets_config *conf) {
  unsigned int cache_sets = conf->cache_size / LINE_SIZE / conf->cache_way / conf->cache_slices;
  EvsetsElement *tmp = *ptr, *prev = NULL;
